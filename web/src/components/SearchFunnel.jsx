    ...
    <div className="space-y-1.5">
      {steps.map((step, i) => (
        <div key={i} className="flex items-center gap-2">
          <span className="text-xs text-slate-500 w-16 text-right flex-shrink-0">{step.label}</span>
          <div className="flex-1 bg-slate-100 rounded-full h-4 overflow-hidden">
            <div
              className="h-full bg-sky-500 rounded-full transition-all duration-500 flex items-center justify-end pr-1"
      ...
      {process.primary_criterion && (
        <p className="text-xs text-slate-500 mt-1">
          决策依据：<span className="font-medium text-slate-700">{process.primary_criterion}</span>
        </p>
      )}
    </div>
  )
